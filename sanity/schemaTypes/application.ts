...
/**
 * Application Scenario schema — 应用场景
 * 用于 Applications section + /applications/[slug] 详情页
 */
export const application = defineType({
  name: "application",
  title: "Application Scenario / 应用场景",
  type: "document",
  fields: [
    defineField({
      name: "slug",
      title: "URL Slug",
      type: "slug",
      description: "URL 路径，例如 smart-factory",
      options: {source: "titleEn", maxLength: 96},
      validation: (Rule) => Rule.required(),
    }),
    defineField({name: "order", title: "Display Order / 显示顺序", type: "number"}),
    defineField({name: "code", title: "Code Label / 代号标签", type: "string"}),
    defineField({
      name: "figureNumber",
      title: "Figure Number / 图示编号",
      type: "string",
      description: '例如 "FIG. 01"',
    }),
    defineField({
      name: "figureLabelEn",
      title: "Figure Label (EN)",
      type: "string",
      description: '例如 "Industrial IoT"',
    }),
    defineField({name: "figureLabelZh", title: "Figure Label (ZH) / 图示标签", type: "string"}),
  ...
  preview: {
    select: {title: "titleEn", subtitle: "figureNumber", media: "heroImage"},
  },
})
